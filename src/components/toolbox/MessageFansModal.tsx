...
export function MessageFansModal({ open, onOpenChange, onBack }: MessageFansModalProps) {
    ...
    const handleSendNow = async () => {
        ...
        try {
            ...
            toast({
                title: 'Message sent!',
                description: response.recipient_count === 1
                    ...
                    <div className="flex flex-1 flex-col">
                        ...
                        <div className="flex gap-2 border-t bg-muted/30 p-3">
                            <Button
                                ...
                                className="flex-1"
                            >
                                {savingDraft ? <Loader2 className="mr-2 h-3.5 w-3.5 animate-spin" /> : <Save className="mr-2 h-3.5 w-3.5" />}
                                Save Draft
                            </Button>
                            <Button
                                onClick={handleSendNow}
                                disabled={sendingNow || !subject.trim() || !messageBody.trim() || selectedListIds.size === 0}
                                size="sm"
                                className="flex-1"
                            >
                                {sendingNow ? <Loader2 className="mr-2 h-3.5 w-3.5 animate-spin" /> : <Send className="mr-2 h-3.5 w-3.5" />}
                                Send Now
                            </Button>
                        </div>
                    </div>
